...
-- ============================================
-- 5. 修正 t_menu.type 的默认值与存量数据
-- ============================================
-- 该列存的是字符串枚举 CATALOG/MENU/BUTTON，旧默认值 '0' 与取值不符。
ALTER TABLE `t_menu` ALTER COLUMN `type` SET DEFAULT '';

-- 回填旧默认值留下的 '0' 与未填写的空串：
-- 有子节点的为目录，其余为菜单（与前端 route.children?.length ? CATALOG : MENU 的推断一致）。
UPDATE `t_menu` AS m
SET m.`type` = CASE
    WHEN EXISTS (SELECT 1 FROM (SELECT `id`, `parent_id` FROM `t_menu`) AS c WHERE c.`parent_id` = m.`id`)
        THEN 'CATALOG'
    ELSE 'MENU'
END
WHERE m.`type` IN ('', '0');
